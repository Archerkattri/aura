...
14. run `aura benchmark-reference outputs/native-demo.aura --include-ablations`;
15. run `aura benchmark-plan` and fill result-producing harnesses without overclaiming metrics;
16. run `aura ingest-adapters` and keep future source adapters evidence-based;
17. extend the tiny JSON/ASCII/binary PLY 3DGS export fixtures if needed;
