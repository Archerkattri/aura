...
def test_torch_carrier_kernel_report_is_a_production_readiness_gate():
    ...
    assert report["referenceOnlyCarrierCount"] == 5
    assert report["autogradCarrierCount"] == 2
...
def test_torch_kernel_report_cli_prints_readiness_json():
    ...
    assert payload["referenceOnlyCarrierCount"] == 5
    assert payload["autogradCarrierCount"] == 2
